...
print(z_conv1.shape)
bn1 = tf.layers.batch_normalization(z_conv1, training=is_training)
a_conv1 = tf.nn.relu(bn1)
...
print(z_conv2.shape)
bn2 = tf.layers.batch_normalization(z_conv2, training=is_training)
a_conv2 = tf.nn.relu(bn2)
...
print(z_conv3.shape)
bn3 = tf.layers.batch_normalization(z_conv3, training=is_training)
a_conv3 = tf.nn.relu(bn3)
max_pool3 = tf.nn.max_pool(a_conv3,ksize=[1,2,2,1],strides=[1,2,2,1], padding='SAME')


#Convolution layer 4
w_conv4 = tf.get_variable("w_conv4",shape = [3,3,128,512])
b_conv4 = tf.get_variable("b_conv4",shape = [512])
...
flat1 = tf.reshape(max_pool4,[-1,18432])
...
# dropout
drp = tf.nn.dropout(a_fc1,keep_prob = 0.3)
