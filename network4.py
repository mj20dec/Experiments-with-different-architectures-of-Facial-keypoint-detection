...
#Convolution layer 2
w_conv2 = tf.get_variable("w_conv2",shape = [3,3,64,128])
b_conv2 = tf.get_variable("b_conv2",shape = [128])
...
max_pool2 = tf.nn.max_pool(bn2,ksize=[1,2,2,1],strides=[1,2,2,1], padding='SAME')

flat1 = tf.reshape(max_pool2,[-1,4608])

#Fully connected 
w_fc1 = tf.get_variable("w_fc1",shape = [4608,512])
b_fc1 = tf.get_variable("b_fc1",shape = [512])
fc1 = tf.matmul(flat1,w_fc1)+b_fc1
a_fc1 = tf.nn.relu(fc1)

# dropout
drp = tf.nn.dropout(a_fc1,keep_prob = 0.4)
