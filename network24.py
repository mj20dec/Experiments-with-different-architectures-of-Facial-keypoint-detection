...
def augment(x, y):
	...
	return (np.copy(x_all), np.copy(y_all))
...
flat1 = tf.reshape(bn3,[-1,4096])

# dropout
drp = tf.nn.dropout(flat1,keep_prob = 0.5)

#Fully connected 
w_fc1 = tf.get_variable("w_fc1",shape = [4096,512])
b_fc1 = tf.get_variable("b_fc1",shape = [512])
fc1 = tf.matmul(drp,w_fc1)+b_fc1
a_fc1 = tf.nn.relu(fc1)
bn4 = tf.layers.batch_normalization(a_fc1, training=is_training)

# dropout
drp1 = tf.nn.dropout(bn4,keep_prob = 0.3)
