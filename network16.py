...
#Convolution layer 1
w_conv1 = tf.get_variable("w_conv1",shape = [3,3,1,20])
b_conv1 = tf.get_variable("b_conv1",shape = [20])
...
#Convolution layer 2
w_conv2 = tf.get_variable("w_conv2",shape = [3,3,20,40])
b_conv2 = tf.get_variable("b_conv2",shape = [40])
...
#Convolution layer 3
w_conv3 = tf.get_variable("w_conv3",shape = [3,3,40,80])
b_conv3 = tf.get_variable("b_conv3",shape = [80])
...
#Convolution layer 4
w_conv4 = tf.get_variable("w_conv4",shape = [3,3,80,100])
b_conv4 = tf.get_variable("b_conv4",shape = [100])
z_conv4 = tf.nn.conv2d(max_pool3,w_conv4,strides = [1,1,1,1], padding='SAME')+b_conv4
print(z_conv4.shape)
a_conv4 = tf.nn.relu(z_conv4)
max_pool4 = tf.nn.max_pool(a_conv4,ksize=[1,2,2,1],strides=[1,2,2,1], padding='SAME')


flat1 = tf.reshape(max_pool4,[-1,3600])

# dropout
drp = tf.nn.dropout(flat1,keep_prob = 0.3)

#fully connected 2
w_fc2 = tf.get_variable("w_fc2",shape = [3600,30])
